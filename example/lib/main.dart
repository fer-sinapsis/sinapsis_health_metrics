...
class _MyAppState extends State<MyApp> {
  ...
  Future<void> initPlatformState() async {
    if (!mounted) return;
    final resultMessage = await testFnThatRequiresPermissions(testGetStepsCountByInterval);
    setState(() {
  ...
  Future<String> testGetStepsCountByInterval() async {
    final startDate = DateTime.now().subtract(const Duration(hours: 12)).subtract(Duration(minutes: 25));
    final endDate = DateTime.now();
    const interval = TimeInterval.hour;
    String resultMessage;
    try {
      final results = await HealthMetricsObservers.getStepsCountByIntervals(startDate, endDate, interval);
  ...
  Future<void> testLastSaved() async {
    ...
    try {
      ...
      await HealthMetricsObservers.updateLastDateSaved(newDateInMilliseconds: nowTime.millisecondsSinceEpoch, metricType: "SLEEP");
      await HealthMetricsObservers.updateLastDateSaved(newDateInMilliseconds: nowTime.subtract(Duration(hours: 6)).millisecondsSinceEpoch, metricType: "STEP");
      final lastSavedStep = await HealthMetricsObservers.getLastDateSaved(metricType: "STEP");
      final lastSavedStepDate = DateTime.fromMillisecondsSinceEpoch(lastSavedStep ?? 0);

      final lastSavedSleep = await HealthMetricsObservers.getLastDateSaved(metricType: "SLEEP");
      final lastSavedSleepDate = DateTime.fromMillisecondsSinceEpoch(lastSavedSleep ?? 0);
    ...
    try {
      ...
      if (permissionsGranted) {
        ...
        await HealthMetricsObservers.createStepCountObserver(
            apiUrl: "https://api.stage.trywecare.com",
            endpoint: "/api/profile/public/metrics",
            userId: Platform.isIOS ? "ccbbbd98-a9a2-435e-a694-3e3004a71e73" : "ccbbbd98-a9a2-435e-a694-3e3004a71e73",
            userApiKey: Platform.isIOS ? "791e033d-4134-4b27-99f7-955e4624ba67" : "791e033d-4134-4b27-99f7-955e4624ba67",
  ...
  Future<String> testFnThatRequiresPermissions (Future<String> Function() fn) async {
      String resultMessage;
      try {
        final permissionsGranted = await requestPermissionsForSteps();
        if (permissionsGranted) {
