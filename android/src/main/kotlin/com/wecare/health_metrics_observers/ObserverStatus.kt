...
class ObserverStatus {
    companion object {
        ...
        const val apiUrlKey = "api_url"
        const val migratedToShcrApiKey = "migrated_to_sharecare_api"
        ...
        fun getObserverMetadata(context: Context, observerMetadataKey: String): String? {
            val sharedPref =
                context.getSharedPreferences(AppConstants.sharedPrefId, Context.MODE_PRIVATE)
            return sharedPref.getString(observerMetadataKey, null)
        }

        fun updateObserverMetadata(context: Context, observerMetadataKey: String, metadata: String){
            val sharedPref =
                context.getSharedPreferences(AppConstants.sharedPrefId, Context.MODE_PRIVATE)
            with (sharedPref.edit()) {
                putString(observerMetadataKey, metadata)
                apply()
            }
        }
        ...
        fun updateApiUrl(context: Context, newApiUrl: String){
            val sharedPref = context.getSharedPreferences(AppConstants.sharedPrefId, Context.MODE_PRIVATE)
            with (sharedPref.edit()) {
                putString(apiUrlKey, newApiUrl)
                apply()
            }
        }
        ...
        fun getApiUrl(context: Context): String? {
            val sharedPref =
                context.getSharedPreferences(AppConstants.sharedPrefId, Context.MODE_PRIVATE)
            return sharedPref.getString(apiUrlKey, null)
        }

        fun getMigrated(context: Context) : Boolean {
            val sharedPref = context.getSharedPreferences(AppConstants.sharedPrefId, Context.MODE_PRIVATE)
            return sharedPref.getBoolean(migratedToShcrApiKey, false)
        }

        fun updateMigrated(context: Context, newValue: Boolean){
            val sharedPref = context.getSharedPreferences(AppConstants.sharedPrefId, Context.MODE_PRIVATE)
            with (sharedPref.edit()) {
                putBoolean(migratedToShcrApiKey, newValue)
                apply()
            }
        }
        ...
        fun getStatusMap(context: Context): Map<String, Any?> {
            ...
            return mapOf<String, Any?>(
                ...
                "api_url" to ObserverStatus.getApiUrl(context),
                "migrated_to_shrcr_api" to ObserverStatus.getMigrated(context),
            )
        }
    }
}